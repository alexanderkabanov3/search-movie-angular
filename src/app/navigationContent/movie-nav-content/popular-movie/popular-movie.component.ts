...
export class PopularMovieComponent implements OnInit, DoCheck {
  public moviesArr = [];
  public pages = 1;
  ...
  async fetchingList() {
    const request: any = await this.httpMovie
      .get(
        `https://api.themoviedb.org/3/discover/movie?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${this.pages}&with_watch_monetization_types=flatrate`
      )
      .toPromise();
  ...
  async filterAlone() {
    const filt: any = await this.httpFiler
      .get(
        `https://api.themoviedb.org/3/discover/movie?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&language=en-US&sort_by=${this.sortService.sortValue}&include_adult=false&include_video=false&page=${this.pages}&with_genres=${this.sortService.genres}&with_watch_monetization_types=flatrate`
      )
      .toPromise();
