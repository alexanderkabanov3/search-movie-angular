...
@Component({
  ...
  styleUrls: ['./playing-movie.component.scss'],
  animations: [
    trigger('item', [
...
export class PlayingMovieComponent implements OnInit, DoCheck {
  public moviesArr = [];
  public pages = 1;
  ...
  async fetchingList() {
    const request: any = await this.httpMovie
      .get(
        `https://api.themoviedb.org/3/movie/now_playing?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&language=en-US&page=${this.pages}`
      )
      .toPromise();
