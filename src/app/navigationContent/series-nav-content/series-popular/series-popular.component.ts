...
@Component({
  selector: 'app-series-popular',
  templateUrl: './series-popular.component.html',
  styleUrls: ['./series-popular.component.scss'],
  animations: [
    trigger('item', [
...
export class SeriesPopularComponent implements OnInit, DoCheck {
  public moviesArr = [];
  public pages = 1;
  ...
  async fetchingList() {
    const request: any = await this.httpMovie
      .get(
        `https://api.themoviedb.org/3/discover/tv?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&language=en-US&sort_by=popularity.desc&page=${this.pages}&include_null_first_air_dates=false&with_watch_monetization_types=flatrate`
      )
      .toPromise();
