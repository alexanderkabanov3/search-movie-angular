...
import {MediaItem, Cast, Results} from '../../interfaces/fetchingResults';
...
export class DetailMovieComponent implements OnInit, DoCheck {
  public id: number;
  ...
  ngOnInit(): void {
    ...
    this.router.params.subscribe((params: Params) => {
      ...
      this.fetchItem(this.id).then((data: MediaItem) => {
        this.item = data;
      ...
      this.getCredits(this.id).then((data: Cast) => {
        this.cast = data.cast;
      });

      this.getSimilar(this.id).then((data: Results) => {
      ...
      this.ageRait(this.id).then((data: any) => {
  ...
  async getSimilar(id): Promise<object> {
    return await this.httpSimilar
      .get(
        `https://api.themoviedb.org/3/movie/${id}/similar?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&language=en-US&page=1`
      )
      .toPromise();
  }
  ...
  addFavorite(event): void {
    this.favoriteService.addMovieItem(event);
  }
