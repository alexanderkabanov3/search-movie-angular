import {HttpClient, HttpHeaders} from '@angular/common/http';
import {Component, OnDestroy, OnInit} from '@angular/core';
import {ActivatedRoute, Params} from '@angular/router';
import {FavoriteService} from 'src/app/shared/services/favorite.service';
import {animate, style, transition, trigger} from '@angular/animations';
import {BreakpointObserver, BreakpointState} from '@angular/cdk/layout';
import {
  MediaItem,
  Cast,
  Results,
} from '../../shared/interfaces/fetchingResults';
import {Store} from '@ngrx/store';
import {
  loggedInSelector,
  usernameSelector,
} from '../../shared/store/reducers/user';
import {
  dataLeft,
  trailerDoesNotExist,
  trailerLeaveAction,
  trailerOpenAction,
  trailerStream,
} from '../../shared/store/reducers/trailer';
import {TrailerService} from '../../shared/services/trailer.service';
import {Subscription} from 'rxjs';

@Component({
  selector: 'app-detail-movie',
  templateUrl: './detail-movie.component.html',
  styleUrls: ['./detail-movie.component.scss'],
  animations: [
    trigger('item', [
      transition(':enter', [style({opacity: 0}), animate('850ms ease-out')]),
    ]),
  ],
})
export class DetailMovieComponent implements OnInit, OnDestroy {
  public id: number;
  public bgImgPath: string;
  public preLoader = true;
  public item;
  public genres: string;
  public ageAccess: string;
  public cast = [];
  public similar = [];
  public modal = false;
  public commentsAmount = 0;
  public commentsArray = [];
  public comment = '';
  public barValue = 25;
  public titleFontSize = '15';
  public outerStrokeWidth = 5;
  public unitsFontSize = '12';
  public currentUser$ = this.store.select(usernameSelector);
  public checkedIn$ = this.store.select(loggedInSelector);
  private trailerStream$ = this.store.select(trailerStream);
  private trailerStreamSubscription$: Subscription;
  public trailerExist = true;
  public type: string;

  constructor(
    private httpMovie: HttpClient,
    private httpSimilar: HttpClient,
    private httpCredits: HttpClient,
    private router: ActivatedRoute,
    public favoriteService: FavoriteService,
    private breakpointObserver: BreakpointObserver,
    private postComment: HttpClient,
    private getAllCommentsHttp: HttpClient,
    private deleteCommentHttp: HttpClient,
    private store: Store,
    private trailerService: TrailerService
  ) {}

  ngOnInit(): void {
    window.scrollTo(0, 0);

    this.trailerStreamSubscription$ = this.trailerStream$.subscribe((event) => {
      this.modal = event.open;
      if (!event.trailerExist) {
        this.trailerExist = false;
      }
    });

    this.router.params.subscribe((params: Params) => {
      this.trailerExist = true;
      window.scrollTo(0, 0);
      this.preLoader = true;

      this.id = +params.id;
      this.type = params.type;

      this.getAllComments();

      this.trailerService.fetchTrailer(this.id, this.type);

      this.fetchItem(this.type, this.id);
      this.getCredits(this.type, this.id);
      this.getSimilar(this.type, this.id);
      this.ageRait(this.type, this.id);
    });

    this.mediaQueries();
  }

  fetchItem(type: string, id: number): void {
    this.httpMovie
      .get(
        `https://api.themoviedb.org/3/${type}/${id}?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&append_to_response=videos`
      )
      .subscribe((data: MediaItem) => {
        this.item = data;

        const genre = [];
        this.item.genres.forEach((element) => {
          genre.push(element.name);
        });
        this.genres = genre.join(' ');

        data.backdrop_path !== null
          ? (this.bgImgPath = `https://image.tmdb.org/t/p/original${data.backdrop_path}`)
          : (this.bgImgPath =
              'data:image/jpeg;base64,');
      });
  }

  getSimilar(type: string, id: number): void {
    this.httpSimilar
      .get(
        `https://api.themoviedb.org/3/${type}/${id}/similar?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&language=en-US&page=1`
      )
      .subscribe((data: Results) => {
        this.similar = data.results;

        setTimeout(() => {
          this.preLoader = false;
        }, 500);
      });
  }

  getCredits(type: string, id: number): void {
    this.httpCredits
      .get(
        `https://api.themoviedb.org/3/${type}/${id}/credits?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&language=en-US`
      )
      .subscribe((data: Cast) => {
        this.cast = data.cast;
      });
  }

  ageRait(type: string, id: number): void {
    this.httpMovie
      .get(
        `https://api.themoviedb.org/3/${type}/${id}?api_key=f4a143e6e64636aa4b0cd6bec7236ad4&append_to_response=release_dates`
      )
      .subscribe((data: any) => {
        if (data.release_dates !== undefined) {
          data.release_dates.results.forEach((element) => {
            if (element.iso_3166_1 === 'US') {
              if (element.release_dates[0].certification !== undefined) {
                this.ageAccess = element.release_dates[0].certification;
              }
            }
          });
        }
      });
  }

  open(): void {
    this.store.dispatch(trailerOpenAction());
  }

  addFavorite(type: string, id: number): void {
    this.favoriteService.addMovieItem(type, id);
  }

  leaveComment(): void {
    const comObj = {
      text: this.comment,
      userName: localStorage.getItem('userName'),
    };

    const token = `Bearer ${localStorage.getItem('token')}`;
    const header = new HttpHeaders().set('Authorization', token);

    this.postComment
      .post(
        `https://search-movie-server.herokuapp.com/api/comments/movieoverview/${this.id}`,
        comObj,
        {
          headers: header,
        }
      )
      .subscribe(
        (observer: any) => {
          if (observer.status === 'success') {
            this.getAllComments();
          }
        },
        (error) => {
          console.log(error);
        }
      );

    this.comment = '';
  }

  getAllComments(): void {
    this.getAllCommentsHttp
      .get(
        `https://search-movie-server.herokuapp.com/api/comments/movieoverview/${this.id}`
      )
      .subscribe(
        (observer: any) => {
          this.commentsArray = observer.data.reverse();
          this.commentsAmount = this.commentsArray.length;
        },
        (error) => {
          console.log(error);
        }
      );
  }

  deleteComment(event: number): void {
    const token = `Bearer ${localStorage.getItem('token')}`;
    const header = new HttpHeaders().set('Authorization', token);

    this.deleteCommentHttp
      .delete(
        `https://search-movie-server.herokuapp.com/api/comments/movieoverview/${this.id}/${event}`,
        {
          headers: header,
        }
      )
      .subscribe(
        (observer: any) => {
          if (observer.status === 'success') {
            this.getAllComments();
          }
        },
        (error) => {
          console.log(error);
        }
      );
  }

  mediaQueries(): void {
    this.breakpointObserver
      .observe(['(max-width: 1024px)'])
      .subscribe((state: BreakpointState) => {
        if (state.matches) {
          this.barValue = 20;
          this.titleFontSize = '12';
          this.outerStrokeWidth = 4;
        }
      });

    this.breakpointObserver
      .observe(['(max-width: 790px)'])
      .subscribe((state: BreakpointState) => {
        if (state.matches) {
          this.barValue = 16;
          this.titleFontSize = '8';
          this.outerStrokeWidth = 2;
          this.unitsFontSize = '8';
        }
      });

    this.breakpointObserver
      .observe(['(max-width: 650px)'])
      .subscribe((state: BreakpointState) => {
        if (state.matches) {
          this.barValue = 13;
          this.titleFontSize = '8';
          this.outerStrokeWidth = 2;
          this.unitsFontSize = '8';
        }
      });
  }

  ngOnDestroy(): void {
    this.store.dispatch(trailerLeaveAction());
    this.store.dispatch(dataLeft());
    this.trailerStreamSubscription$.unsubscribe();
  }
}
